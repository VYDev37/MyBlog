<x-app-layout>
    <div class="py-6 sm:py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-2xl sm:rounded-3xl border border-gray-100">
                <div class="p-6 sm:p-10 lg:p-12">
                    <!-- Title -->
                    <h1
                        class="text-2xl sm:text-3xl lg:text-4xl font-black text-gray-900 tracking-tight leading-tight mb-6 sm:mb-8">
                        {{ $post->title }}
                    </h1>

                    <!-- Profile & Meta Info -->
                    <div class="flex items-center gap-4 mb-4 sm:mb-6 pb-4 sm:pb-5 border-b border-gray-100">
                        ...
                        @else
                            <div
                                class="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-indigo-50 text-indigo-500 flex items-center justify-center ring-2 ring-gray-50 shrink-0">
                        ...
                        <div class="flex flex-col justify-center">
                            <a href="{{ route('profile.show', ['user' => $post->user->username]) }}"
                                class="text-base sm:text-lg font-semibold text-gray-900 leading-snug">
                                {{ $post->user->name }}
                            </a>
                            <div
                                class="flex items-center flex-wrap gap-x-2 gap-y-1 mt-0.5 text-xs sm:text-sm text-gray-500 font-medium">
                    ...
                    <div class="prose prose-lg max-w-none">
                        @if($post->image)
                            <div
                                class="w-full relative aspect-video rounded-xl sm:rounded-2xl overflow-hidden shadow-lg border border-gray-100 bg-gray-50 group">
                                <img src="{{ $post->imageUrl() }}" alt="{{ $post->title }}"
                                    class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 hover:scale-105">
                            </div>
                        @endif

                        <p class="whitespace-pre-line text-base sm:text-lg text-gray-700 leading-relaxed">
                            {{ $post->content }}
                        </p>

                        <div
                            class="mt-6 w-fit px-3 py-1 text-center rounded-full text-sm font-semibold text-gray-700 bg-gray-100">
